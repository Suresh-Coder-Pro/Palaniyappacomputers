...
import { fetchWithAuth } from "../utils/api"; // Adjust path as needed
import { useNavigate } from "react-router-dom";

const AadharEnrolmentTable = () => {
  ...
  const fetchEntries = async () => {
    setIsLoading(true);
    try {
      const data = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/aadhar`, {}, navigate);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const savedEntry = await fetchWithAuth(
        `${import.meta.env.VITE_API_URL}/aadhar`,
        {
          method: "POST",
  ...
  const handleDelete = async (id) => {
    try {
      await fetchWithAuth(
        `${import.meta.env.VITE_API_URL}/aadhar/${id}`,
        { method: "DELETE" },
        navigate
      );
    ...
    <div className="p-3 sm:p-6 md:p-8 bg-gray-100 min-h-screen text-xs sm:text-sm">
      <div className="w-full sm:w-[90%] md:w-[90%] lg:w-full mx-auto">
        ...
        <form
          ...
          className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6"
        >
          <div>
            ...
            <div className="w-full min-w-[500px]">
              <table className="w-full table-auto text-xs sm:text-sm border border-gray-300">
                <thead className="bg-blue-500 text-white">
                  <tr>
                    {["#", "Count", "Price", "Subtotal", "Time", "Remove"].map(
                      (label) => (
                        <th
                          key={label}
                          className="px-2 sm:px-3 py-2 font-semibold text-left"
                        >
                          {label}
                        </th>
                      )
...
export default AadharEnrolmentTable;
