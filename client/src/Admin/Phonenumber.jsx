...
const PhoneNumber = () => {
  ...
  const fetchEntries = async () => {
    setIsLoading(true);
    try {
      const data = await fetchWithAuth(`${import.meta.env.VITE_API_URL}/phone/`, {}, navigate);
  ...
  const handleDelete = async (id) => {
    try {
      await fetchWithAuth(
        `${import.meta.env.VITE_API_URL}/phone/${id}`,
        { method: "DELETE" },
        navigate
      );
      ...
      <div className="w-full sm:w-[90%] md:w-[90%] lg:w-full mx-auto">
        ...
        <div className="bg-white shadow-lg rounded-lg p-4 sm:p-6 overflow-x-auto">
          <h3 className="text-lg sm:text-xl font-bold text-blue-600 mb-4">
            📜 Today's Phone Enrolment Records
          </h3>
